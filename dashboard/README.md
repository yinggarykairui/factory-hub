...
| 041 | 2026-09-04 | factory-hub | meta | The evening may finally check the seventeen ships nobody was allowed to check — under a header that can never be quoted back as a clean evening | Markdown, MANUAL.md doctrine | 3.79 | [repo](https://github.com/yinggarykairui/factory-hub) | — | meta ([#55](https://github.com/yinggarykairui/factory-hub/issues/55)) | claude-opus-5 |
...
**KPI:** streak: **10** — 2026-08-27 through 2026-09-05, held by the evening for the second night running · **the noon fault has changed shape, and the new shape is worse.** [#93](https://github.com/yinggarykairui/factory-hub/issues/93) was filed on 2026-08-28 for a trigger that *did not fire*, and yesterday's row recorded a second miss. Today's noon shift **fired**: the credentialed `Factory Noon Shift` task ran 12:01–14:08 PT and the scheduler recorded **`SUCCEEDED`**. It wrote nothing — no commit, no comment, no label, no row. A trigger that never fires is detectable by its absence; a trigger that runs for two hours and reports success while shipping nothing is not, because the one signal anybody checks says the factory is fine. Two hours of runtime with zero writes is the signature and nothing in the factory watches for it. Found alongside it and reported to the owner: the account carries **duplicate shift tasks** — an older 2026-07-27 set (`factory-noon-shift`, `factory-evening-shift`, `factory-job-watch`) on `claude-opus-4-8` with **no PAT in their prompts**, beside the 2026-08-03 credentialed pair; both noon tasks fired today and both recorded `SUCCEEDED`, and the old evening task fired one minute after this one and ran in parallel with it. An uncredentialed shift reads everything and writes nothing, which is a precise description of the condition [#60](https://github.com/yinggarykairui/factory-hub/issues/60) was filed about on 2026-08-18, and a standing collision hazard against §15's one-`building`-issue rule. Both findings are on #93; neither is fixable from inside a shift · verified rate: 23/42 — **day 042 is not verified and cannot be** under §16 clause 2: a §11.4 rescue builds what it would verify. Day 041 is unverifiable for the same reason plus clause 5. Day 039 remains unverified — the 2026-09-02 evening left no trace (#60). Day 040 **verified** at `fcb1fec` — [EVENING VERIFIED day-040](https://github.com/yinggarykairui/factory-hub/issues/54#issuecomment-5543208394); day 038 at `57faf14`, day 037 at `bfb0fa8`, day 036 at `40bc8a2`. Days 019–022, 024, 026 and 028 stand unlabelled; **evidence complete for 004–010, relabelling still owed** — [#116](https://github.com/yinggarykairui/factory-hub/issues/116)'s third residual. No verification debt was paid tonight: §11's paragraph reserves 45 minutes before pencils-down for §2.6, and after two fix cycles and three review passes there were not two spare minutes, let alone forty-five · avg rubric score: **4.25** (42 rows; 174.06 over 41 was 4.2454, and day 042 enters at **4.50** — delight 4 · clarity 4.50 · readme 4.50 · scope 5.00. That is the mean of the **two votes taken on a shipping tree**: the cycle-2 re-verify at `a6ef6f7` and the final vote at `df5db4f`. Cycle 1's three critics graded `0312963`, which two fix cycles then replaced, so their scores are not averaged in — grading a tree that no longer exists is not evidence about the one that shipped) · demos alive: 17/17, unchanged · clean evenings: **11**, unchanged, and the live run stays **0**. Tonight cannot move it and did not try: clause 2 disqualifies a rescue evening before clause 5 gets a turn · **evening 2026-09-05:** two jobs, and the manual orders them in this sequence. §11.3 first — #55 carried `needs-retry` from last night, asking for one specific thing, a re-vote of MANUAL.md §11's verification-debt paragraph at `c6fac56` where two fixes shipped unreviewed under directive 4. Tonight's shift did not write them, so it could grade them. **REJECT, and the label stays.** Fix 1 was meant to delete a hard-coded debt count and instead edited the other half of the sentence: :853 still reads *"Seventeen of the forty ships"*, sixty-nine lines above :933 explaining why any hard-coded "seventeen of forty" is false within the hour — and the same commit wrote the count into a **third** place, the day-041 row directly above this one. Fix 2's Skip rests on *"a repo revisited since the day under test serves a later sha, so the marker cannot be there"*, and markers **survive** revisits: days 039 and 040 are increments to the same pages, so most such strings are still served, which leaves a shift that finds day 020's marker present a licensed route to the irreversible `verified` the fix exists to prevent. Five specific remedies are on #55 · then §11.4 — nothing had landed, so the evening built. §4's pick order gave the oldest `queued`, #56 at twenty-one days: five of `snake-flee`'s ten measured day-022 residuals, six excluded by name in the spec. **The finding worth carrying past the build is about the test suite, twice over.** The first suite grew 120 → 145 and a mutation run found **six source mutants that left it green** — including `state = game.over ? 'over'`, which reverts the whole win feature and draws the death card on a win. The second grew to 168 and closed those, and a fresh mutation run found why they had escaped: it was asserting against **its own hand-written copy of the page chrome**. Renaming `<canvas id="canvas">` in `index.html`, which leaves the shipped page a dead board with no game at all, still read **168/168 green**; so did removing `role="status"`, so did changing the card's copy. The suite now fetches and drives the real `index.html` in a frame whose viewport it sets itself — 177 rows over http, and over `file://` it reports 145 passed, 0 failed, **1 skipped** with the row naming the reason and the fix rather than passing quietly. It had also become **height-dependent** in between: 167/168 in any window ≤360 px tall, a red gate on a laptop with a short window, caught by the cycle-2 re-verify. Cycle 1: correctness APPROVE, ux APPROVE, hygiene **REJECT** on a must-pass line — the provenance footer read `Day 022` on a README documenting day-042 work. Cycle 2 re-verify REJECT, fixable. Final vote APPROVE. `loop_cap: 3` spent, so four residuals are filed to [#117](https://github.com/yinggarykairui/factory-hub/issues/117) rather than fixed unreviewed — which is the lesson day 041 paid for. 22 commits, `3091762..df5db4f`, authorship asserted over the run's whole range before each of three pushes (§9.2); **zero `Co-Authored-By` trailers** (#112). `origin` never held the token: credentials passed in a one-off URL per push, piped through a redactor (#68). gitleaks 8.28.0 clean over 51 commits in `git` mode and clean over the worktree, behind a control of freshly randomised values that fired 3 findings including from history after a commit-then-delete. Demo confirmed at the shipped sha, not merely "loads": `WebFetch` with a cache-buster returns `score 0 best 0★`, and that **★ is unique to today's tree** — it enters in `7141161`. Shipped sha `df5db4f`
