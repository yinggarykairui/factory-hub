...
**KPI:** streak: **14** — 2026-08-27 through 2026-09-09; the noon shift fired 12:01 PT and reached the API plane on its first try by the documented bypass, `curl --noproxy '*'` for every API write and `env -u https_proxy … git -c http.proxy= push` for all four pushes. **[#93](https://github.com/yinggarykairui/factory-hub/issues/93) is not closed by two good runs** — its record stands unchanged: 08-26 and 08-27 the trigger did not fire; 09-05 a two-hour run that wrote nothing; 09-06 2 m 42 s; 09-07 3 m 52 s with zero writes, every one reported `SUCCEEDED`; the uncredentialed 2026-07-27 twin `factory-noon-shift` and the parallel-firing `factory-evening-shift` are also still on it, neither fixable from inside a shift · verified rate: 25/46 — **day 046 verified by the 2026-09-09 evening shift at `cea938e`**, the tip after that evening's 22 polish commits, with the §11.2 spot-check run last: `origin/HEAD` equal to the sha under test, a cache-busted `WebFetch` returning four literals the evening introduced and `git grep -F` confirms absent at `764e949` while `BINARY_SAMPLE` (which `764e949` had) is gone, `screenshot.png` present and re-shot byte-identical from the tip, gitleaks 8.21.2 clean over 74 commits and over the worktree behind a control of freshly generated values that fired first, and the random must-pass draw landing on **usable at phone width** — answerable rather than unavailable for once, because Chromium and Playwright are present in this sandbox: 0 horizontal overflow and 0 elements past the viewport at 320×568, 360×640 and 390×844 under touch emulation, 44 px tap targets, the example rendering 23 steps, and `{oops` answered while the run stayed at 23. Evidence: [#59](https://github.com/yinggarykairui/factory-hub/issues/59#issuecomment-5613773746). **The verification ledger, carried forward unchanged** (a later evening reads this before picking a debt candidate, §11): day 044 is not verified and cannot be — §16 clause 2, its evening built what it would verify; day 043 stands unverified for clauses 2 and 5; day 042 for clause 2; day 041 for 2 and 5; day 039 for [#60](https://github.com/yinggarykairui/factory-hub/issues/60). Verified with quotable evidence: day 046 at `cea938e`; day 040 at `fcb1fec`; day 038 at `57faf14`; day 037 at `bfb0fa8`; day 036 at `40bc8a2`; and day 026 (`critic-loop`) at `59a6820` under §11's verification-debt paragraph, all four sub-checks run and passed. Days 019–022, 024, 028 and 045 stand unlabelled; **evidence complete for 004–010, relabelling still owed** — [#116](https://github.com/yinggarykairui/factory-hub/issues/116)'s third residual, and explicitly *not* a debt candidate under §11 · avg rubric score: **4.25** (46 rows; 191.11 over 45 was 4.2469, and day 046 enters at **4.25**, giving 195.36 over 46 = 4.2470). Per line, from each critic's **final** position and attributed rather than averaged into anonymity: delight 4/4/4 → 4.00 (correctness/ux/hygiene), clarity 4/4/4 → 4.00, readme 4/4/5 → 4.33, scope 5/5/4 → 4.67 · demos alive: 17/17, unchanged · clean evenings: **11** and the live run **0**, both left exactly as the noon shift wrote them — §11 forbids a shift that verified today from deciding whether today counts, so whether the 2026-09-09 evening was clean under §16's five clauses is a later shift's call against this record, not this one's claim · **day 046:** the queue's oldest `queued` issue, [#59](https://github.com/yinggarykairui/factory-hub/issues/59) (2026-08-18), the residuals day 025 left and the 2026-08-18 evening added to. The spec took five and **shipped four**, and the day's content is that **two of the five were wrong and only measurement said so**. **N12 — hide the signpost above 46rem — was built and then reverted.** The filing was right that the line is redundant at 1280×800 and wrong that a *width* rule can express it: the premise is about height. At viewport height 640 the `TRANSCRIPT JSON` label sits **654–733 px** down at every width from 737 to 1920; at 700 tall it is still below the fold at 737, 800 and 900; the Render button does not clear the fold until about **1031 px** of viewport. A maximised browser on a 1366×768 laptop is ~657 px — wide of the old query and exactly the size that needs the line. Done-checklist item 7 asserts the opposite and is **not met as written**; the measurement now lives in the CSS so the next shift reads it before re-filing · **N3 was fixed in the direction that cost the reader, then re-fixed.** Unifying the container-driven grid onto the script's 46rem handed the 704–736 px band the *phone* layout: 720×800 went from **901 px** of document, 3 rows and the detail card at y=305, to **1219 px**, 4 rows and the card at **y=624** — a page that had fitted one screen no longer did. Moving **both** queries to `43.99rem` gives that band two columns and the wide rules together: **1144 px, 5 rows, card at 381**, better than the rejected build on every measure and better than base on two. Fourteen widths swept, **zero disagreements** between track count, `matchMedia` and the timeline's max-height; base disagreed at four · **The binary guard shipped wrong twice before it shipped right.** v0 tested NUL *or* a U+FFFD density over 1% in the first 4096 characters, and the density half was wrong in both directions: a valid transcript whose `tool_result` legitimately carries replacement characters was refused **and padding the same content past 4096 characters flipped the verdict**, while real PDFs sit at 0.10% U+FFFD and sailed through. NUL alone closed the false positives and left the blocker — a genuine TeX Live PDF put **10,945 characters** into the box, **65 of 546 PDFs on this machine (12%) carry no NUL in their first 4096 bytes**, and a NUL at character 6,039 evaded the window entirely. The test that works is the one JSON supplies: a raw C0 control other than tab, LF and CR is invalid inside a JSON string, and an exhaustive check of all 29 such codepoints in five syntactic positions found `JSON.parse` rejecting **145 of 145**, so nothing parseable can carry one. Whole-text scanning for that class costs **7.8 ms at the 12,000,000-character input cap**, so the sample window went too. Final matrix: PNG, JPEG, GIF, WebP, gzip, bzip2, ZIP, tar, sqlite, BOM-less UTF-16LE, two real PDFs and a 5,000-byte NUL-free byte stream all refused by name with the box at length 0; the bundled example, a 400×U+FFFD transcript, a cp1252 transcript, a UTF-16-with-BOM transcript, one with literal tabs and CRLF and one with *escaped* control sequences all still accepted. An all-ASCII `reportlab` PDF still reaches the box — the README says so rather than claiming otherwise · **Two numbers the page told the reader about their own machine were false.** v0 cleared the box above `BUSY_CHARS` (100,000) saying *"typing in a box that size costs about a fifth of a second a keystroke"*; three independent harnesses put the cost at 100,000 characters at **one vsync frame, indistinguishable from an empty box** — about **8× overstated** — while the same sentence was ~2× *understated* at 2 MB. The comment justifying the shared constant asserted the two questions *"have the same answer"*; `parseTranscript` costs **10 ms at 147,567 characters and 166 ms at 2,021,167**, a different curve. The constant was split — `BUSY_CHARS` keeps the busy state, `SLOW_BOX_CHARS = 400000` decides the clear, chosen where the median keystroke first crosses the 16.7 ms frame (5.8 ms at 100k, 13.9 at 300k, 16.6–18.0 at 400k, 43 at 1M, 83 at 2M) — and the claim became *"slows every keystroke, and slows further the longer the text gets"*, verified **strictly monotone from 400,001 to the 12,000,000 cap** · **One README clause was still false at the last verification and was cut rather than shipped.** *"and the status says which"* held on two of the three paths it covered: a dropped file over 400,000 characters that then fails to parse empties the box **before** the parse and reports only a character position, never saying the file was not copied in. Pre-existing at `8366b6e` at the old threshold, filed as [#124](https://github.com/yinggarykairui/factory-hub/issues/124) item 1; the sentence went in `764e949` · Review record: **cycle 1 all three critics REJECT** (stale screenshot, the false typing figure, the signpost regression, the band regression, the binary false positives, a tautological refusal message, an over-broad README claim, a `DISTINCT TOOLS 0` contradiction); **cycle 2 ux and hygiene APPROVE, correctness REJECT** on the PDF blocker; **cycle 3 all three APPROVE**. The loop cap is spent · `screenshot.png` re-shot at `606e8ba`, and its framing was **determined rather than guessed** — the day-025 tree was extracted and re-shot until the bytes matched, giving viewport 1280×900 at deviceScaleFactor 2, 2560×1800 — after which two critics independently reproduced the new image **pixel-for-pixel** from HEAD · 18 commits, `8366b6e..764e949`, authorship asserted over the run's whole range before each of four pushes (§9.2), exactly one line, `Kairui Ying <yinggarykairui@gmail.com>`, and **zero `Co-Authored-By` or `Claude-Session` trailers** — the failure day 045 could not repair ([#112](https://github.com/yinggarykairui/factory-hub/issues/112)) did not recur · gitleaks 8.21.2 clean over 52 commits in `git` mode, clean over the worktree and clean under the hub config, behind a positive control of freshly generated values whose fine-grained-PAT body was **asserted at 82 characters** and which fired `github-fine-grained-pat` and `aws-access-token` before any clean result was read · Deploy confirmed at today's tree, not the previous ship's: `WebFetch` with a unique cache-buster returns `app.js` carrying `slows further the longer the text gets`, `does not look like a text file` and `var SLOW_BOX_CHARS = 400000` — all absent at `8366b6e` — and **not** carrying `BINARY_SAMPLE`, which `8366b6e` had. Per `LESSONS.md` that proves which document is served and cannot prove the app runs · **A spec number was wrong all day and no one fixed the example to match it**: the checklist quotes 26 steps for the bundled example in two places; it renders **23**, byte-identical across the whole range. Shipped sha `764e949`
