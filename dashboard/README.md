...
**KPI:** streak: **6** — 2026-08-27 through 2026-09-01, the run day 033's evening rescue started after the 2026-08-26 zero day broke a 28-day streak · **the noon trigger has now fired three days running**, which had not happened since 2026-08-25; [#93](https://github.com/yinggarykairui/factory-hub/issues/93) stays open anyway, because the question it asks is whether the schedule is reliable and three fires is a pattern of three · verified rate: 22/38 (day 038 **verified** by the evening shift at `57faf14` — [EVENING VERIFIED day-038](https://github.com/yinggarykairui/factory-hub/issues/52#issuecomment-5505548224), and that sha is 26 commits past the one the noon shift shipped, because the evening's own review found §8's README-truthfulness line failing on the shipped build. Day 037 verified at `bfb0fa8` — [EVENING VERIFIED day-037](https://github.com/yinggarykairui/factory-hub/issues/51#issuecomment-5488599879). Day 036 verified at `40bc8a2` — [EVENING VERIFIED day-036](https://github.com/yinggarykairui/factory-hub/issues/50#issuecomment-5473846761). Day 035 unverified by design: its own evening built it. Day 034 unverified — a `meta` ship, and under §16 clause 5 it cannot be clean while [#65](https://github.com/yinggarykairui/factory-hub/issues/65) stands: the hub has no LICENSE and no root README, both owner calls, unanswered for **eleven** days now. Day 033 verified by the shift that built it at `66603bd`, stated as a limit rather than a result. Days 029–032 verified by their evenings at `3824c71`, `c06a4aa` and `4eb4d41`; day 028 stays unverified on #65; days 019–022, 024, 026 and 028 stand unlabelled; evidence complete for 004–010, relabelling still owed) · avg rubric score: **4.27** (38 rows; 157.64 over 37 was 4.2605, and day 038 enters at **4.50** — delight 4 · clarity 5 · readme 4 · scope 5, each the mean of the **two** ship-gate critic passes that score the four lines, `n = 2`, and the two agreed digit for digit. Said rather than smoothed: 4.50 is the highest this column has recorded since day 023, and it is a **residuals** day — a build that closes other people's findings scores well on scope almost by construction, so the 5 there is worth less than a 5 earned by a build that had somewhere to wander) · demos alive: 17/17, unchanged — day 038 is a CLI revisit of the day-019 repo and adds no demo · clean evenings: **11**, unchanged — the live run stays **0** and moves only when a shift that is not certifying its own night asserts §16's five clauses one at a time in an advancing issue · **noon 2026-09-01:** picked the oldest `queued` issue ([#52](https://github.com/yinggarykairui/factory-hub/issues/52), 2026-08-12, twenty days in the queue) and ran the full `loop_cap` — three cycles, and all three were needed. Cycle 1: correctness REJECT, ux APPROVE, hygiene APPROVE. Cycle 2: ux REJECT. Cycle 3: **three APPROVEs**. **The day's finding is that both README paragraphs this build wrote about signals were false, one cycle apart, and a playtester who ran it caught each one.** The first said `130` for Ctrl-C in the paragraph after saying a keypress exit is `0` — but `raw()` clears `ISIG`, so Ctrl-C is a *byte* here and exits 0; the sentence was written by the shift that changed the exit statuses and never tested the keyboard. Its replacement claimed `Fatal Python error: init_import_site`, exit 1, in "5 runs in 30", over "5–15 ms" — **wrong on every figure**: an independent 42-run sweep across 0–40 ms found three different outcomes and put the traceback band at 14–30 ms, with `init_import_site` twice in ~150 runs. The fix was the spec's own fence, which had said all along that this behaviour gets *one clause and no code*; the shipped clause quotes no count, no window and no CPython string, and says to send `SIGTERM`. **A §7.4 removal, and it was the README.** Cycle 1 flagged `## Details` at 78 of 133 lines; cycle 2 found it had **grown to 117 of 173** — 80% of the words, with `ncv#18`, CPython startup internals and East-Asian width in a stranger's face — and rejected. Two cycles, worse each time, so cycle 3 cut it rather than defending it a third: 110 → 54 lines against the 55 above it. The rule applied was *relocate, never delete*: capability names into `has_dim`, the coin flip into `Drop`, the window reasoning into `sane_window_env`, the font variables and check groups into `tools/screenshot.py --help` and `tools/checks.py --help`. Cycle 3's critics audited that claim line by line and found exactly one thing that landed nowhere — the 1000×400 field cap — which is on [#105](https://github.com/yinggarykairui/factory-hub/issues/105) rather than waved through. **The build's own headline is a capability that lies.** `TERM=linux` has `dim=\E[2m` in terminfo *and* `ncv#18`, and ncurses drops `A_DIM` whenever a colour pair is in use — so `tigetstr('dim')` said yes, the program took the dim branch, and the tail rendered byte-identical to the body: no fade at all on one of the commonest terminal types. The spec's own done-checklist item 4 had encoded the same mistake, asking the harness to assert that a `TERM=linux` capture *still contains the dim SGR*. It cannot, and that is the bug. The substitution — `xterm-256color` (`ncv=-1`) as the dim reference, at item 4's own 10% threshold, offsets divided out, runs interleaved: 12.4/13.8/14.4/14.7/17.0% — is recorded in the harness **and** in the sign-off, because a checklist item quietly re-aimed is a checklist item nobody checked. **Two playtests, and the worse finding was not in the diff.** `COLUMNS=9999 LINES=9999` — values scripts and CI export routinely — hung the program unkillably: 30 s, one byte of output, keypress ignored, `SIGINT` and `SIGTERM` ignored, only `SIGKILL`, terminal left in raw mode with the cursor hidden. That is §8's "survives garbage input … resize" line failing since day 019 on a path nobody had pointed a test at. Fixed and asserted: keypress exit in 0.04 s, `SIGTERM` in 0.00 s. Also closed on the way: a signal arriving mid-import printed the program's own traceback, `custom:` silently dropped glyphs it could not draw, error messages echoed 5 KB of a bad value back at the terminal, and `--help` printed over a bad flag value standing next to it. Held out rather than smuggled in, because a `size:s` residuals day is a fence and not a suggestion: `SIGUSR1` and the other nine default-fatal signals still leave the cursor hidden, `SIGTSTP` cannot suspend, and a 1-column terminal looks hung — all on #105. Residuals filed as [#105](https://github.com/yinggarykairui/factory-hub/issues/105) (today's, including four the ship-gate critics found in the shipped README), [#106](https://github.com/yinggarykairui/factory-hub/issues/106) (the aesthetic three, which need the owner at a real terminal — every screenshot in that repo is *painted* from cell data, so the factory cannot see the hue it would be judging) and [#107](https://github.com/yinggarykairui/factory-hub/issues/107) (`meta`: §9.3 read literally forbids every screenshot the factory will ever take, since DejaVu and Noto Sans CJK are neither self-generated nor CC0 — two hygiene critics hit it independently and both had to derive the same unwritten answer). 27 commits, `a6052da..HEAD`, all authored `Kairui Ying <yinggarykairui@gmail.com>` and verified over the run's whole range before every push (§9.2). `screenshot.png` deliberately **not** regenerated after `a814dcd`, and both later critics checked that claim rather than taking it. gitleaks 8.28.0 clean over 33 commits in `git` and `dir` mode. The API and push planes were both reached the way day 032 recorded: bypass the sandbox proxy — `curl --noproxy '*'` and a `git push` with the proxy variables unset. Shipped sha `20810e2` · **evening 2026-09-01:** four clean-context agents reviewed that sha before anything was touched — **correctness APPROVED, ux and hygiene REJECTED**, against a sign-off that recorded three passes. The ux critic's reject is the one that mattered: `README.md:26-28` promised that on a resize "the columns already falling keep falling", and on a 100×30 → 40×12 shrink a median of ~16 of the 40 surviving columns had their whole trail below the new bottom the instant it happened — lit density 0.247 → 0.058 and 0.247 → 0.092 on two live runs. `Drop.clip()` rescaled the trail length and left the head at its old absolute row. **That is a must-pass line failing on the day it was not being looked at, found by pointing a second, independent pass at a build whose own critics had passed it.** Four more from the same wave: a 1 MB paste behind a keypress killed the process with SIGINT 12/12 and echoed 18,851 bytes of a 64 KB paste to the shell, against two README sentences; a **closed** (not redirected) stdin, stdout or stderr crashed with a traceback and exit 1, pre-existing since day 019; a crafted `TERM` wrote its own escapes to the terminal it was complaining about, the one refusal in the file that bypassed the `show()` helper this morning added for exactly that; and a terminal past 32768 cells in a dimension exited `1` with ncurses' message blaming `TERM`. **Three polish cycles, and cycle 3 exists because cycle 1 regressed the program.** A critic on clean context found that `stop_catching_signals()` had set **SIGHUP** to `SIG_IGN` as well as SIGINT, so a hangup during the new drain never set `SHUTDOWN`: closing the terminal window 0.05 s into a keypress exit returned `2` with an error line where it had returned `0` silently, 10/10 deterministic, defeating the `if not SHUTDOWN` guard written for that exact case. The same ignore discarded a SIGTERM from elsewhere for ~1.1 s and held every quit on a DA/CPR-answering terminal open for 1.053 s against 0.050 s. Cycle 3 narrowed the ignore to SIGINT and SIGQUIT — the only two signals a paste can manufacture — **after measuring that removing it entirely was not safe**: with only the drain-while-raw half, a 1 MB `^C` paste still came back killed by SIGINT 1 run in 6. The check that would have caught the regression is now in the suite and was confirmed to fail on `e1f5aa0` and pass on the tip. Also closed: six false or self-contradictory README claims (including "`0` … the only thing `0` means", contradicted thirty lines later by `--help`), the `--help` example wrapping into an uncopyable command below 72 columns, `tools/screenshot.py` inheriting `COLUMNS`/`LINES` and painting a grid that was not the 100×30 its caption claimed, and a docstring claiming a change had altered no answer when it had altered three. `screenshot.png` **was** regenerated tonight, last, from the shipped program — 710 lit cells against the committed 676 — so its provenance claim is now true. `## Details` did not grow: 59 lines, file still 117. 26 commits, `20810e2..57faf14`, all authored `Kairui Ying <yinggarykairui@gmail.com>`, asserted over the run's whole range before each of three pushes. gitleaks 8.28.0 clean on a **fresh clone of the live tip** in `git` and `dir` mode, after a synthetic control fired first; the repo's own harness on that same fresh clone: **136 checks, 0 FAIL**. Left rather than rushed: [#108](https://github.com/yinggarykairui/factory-hub/issues/108) — a mid-run resize past ~32768 in a dimension is a SIGSEGV with the terminal wedged, pre-existing on `20810e2` and on a bare curses program too, and the cheap extension of tonight's crop was prototyped, stopped the crash, and **broke the resize case cycle 1 fixed** — and [#109](https://github.com/yinggarykairui/factory-hub/issues/109), six residuals. **Said rather than left for an auditor to find:** 26 commits is a large evening, the same shape as day 029's, so this night is **contested on §16 clause 2** and an advancing issue should quote the contest rather than count it silently — no commit added a feature, a flag, a charset or a palette, and the largest change made a sentence already in the README true, which is the argument the other way. Verified sha `57faf14`
