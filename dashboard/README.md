...
**KPI:** streak: **1** — 2026-09-14, and it is a reset for the second time in four days. **2026-09-13 was a zero day**: the last trace of any shift before this one is 2026-09-13 03:41:08 UTC (2026-09-12 20:41 PT), the 09-12 evening closing its debt payments. No noon ship, no evening verification, no commit after `40faab4`, no issue touched. The streak of **1** the 09-12 ship had just rebuilt ends there and **[#93](https://github.com/yinggarykairui/factory-hub/issues/93) gains an eighth date** — the second in four days, which is why this shift's push notification led with the trigger rather than the ship. Both planes were reached on the first try by the documented bypass, `curl --noproxy '*'` for every API write and `env -u https_proxy … git -c http.proxy= push` for all five pushes, so 09-13 was upstream of the run again · verified rate: **31/49** — the numerator does not move; day 049 enters unverified, and no debt was paid, because the payable set the 09-12 evening left is still empty and a **noon** shift does not pay debt (§11 scopes that paragraph to the evening) · avg rubric score: **4.23** (49 rows; 203.16 over 48 was 4.2325, day 049 enters at **4.00**, giving 207.16 over 49 = 4.2278). Per line, from each critic's **final** position and attributed rather than averaged into anonymity: delight 4/4/4 → 4.00 (correctness/ux/hygiene), clarity 4/4/3 → 3.67, readme 4/4/3 → 3.67, scope 5/4/5 → 4.67. **Hygiene's are its rejecting scores and are carried unchanged**: its two findings were closed after §7's cap under directive 4 and it never re-scored, so the average is, if anything, low — and saying which critic held which number is the only way that stays checkable · demos alive: 17/17, unchanged — a `meta` ship adds no repo and no demo · **clean evenings: 12 ever recorded, and the live run is 0.** The 09-13 zero day breaks it again before it could reach one. · **[#65](https://github.com/yinggarykairui/factory-hub/issues/65) was found closed with its condition unfixed, and is reopened.** It was closed `completed` at `2026-09-13T03:41:08Z` with no comment, **two seconds** after `40faab4` was pushed — a commit whose own message names #65 as still blocking five ships. The tree is unchanged: no root `README.md`, no LICENSE, `license: null` from the API against a description and five topics that are set. §11's *Unverifiable* outcome and §16 clause 5's consequence both read that **open** issue rather than the tree, so with it closed a sound `meta` ship would have drawn a **Fail** that §11 makes sticky. If the close was the owner's and deliberate, one comment on #65 says so · **This row's own correction.** The KPI row has been carrying *"[#94](https://github.com/yinggarykairui/factory-hub/issues/94) or a sha field in §10 is what unblocks five ships"* — days 019–022 and 024. **Both halves are wrong, and day 049 is the ship that found out.** A `sha:` field in §10 is **not retroactive**: those sign-offs sit on closed `shipped` issues that §3 makes immutable, so nothing written into §10 today reaches them. And on §11's own 1.9.1 dry-run record, only **one** of the five was ever held by a missing sha — **day 019**, whose sign-off and dashboard narrative both carry none. That record takes `d74b16f` from **day 020's sign-off** and `287a103` from **day 024's**. So either two sign-offs carry shas two evenings recorded as absent, or the 1.9.1 entry took them from somewhere it did not name — one `gh issue view` on #12 and #16 settles it, and it is **[#130](https://github.com/yinggarykairui/factory-hub/issues/130) item 4**. Until then §11's two live sentences are written to the conservative reading: day 019 is the one demonstrable sha block, days 020–022 and 024 are held by deploy scope, and days 021 and 022 were never walked so their shas are **untested rather than missing**. Recovering the five from git history is #130 item 2 · **The verification ledger, carried forward** (a later evening reads this before picking a debt candidate, §11): day 047 verified at `adbba83`; day 046 at `cea938e`; day 045 at `f67521c`; day 044 at `187b2e2`; day 042 at `df5db4f`; day 040 at `fcb1fec`; day 039 at `e23a18d`; day 038 at `57faf14`; day 037 at `bfb0fa8`; day 036 at `40bc8a2`; day 035 at `6279b06`; day 026 at `59a6820`. **The set, enumerated from the labels and arithmetically closed** — 31 verified + 18 not = 49. **Verified (31):** 001–003, 011–018, 023, 025–027, 029–033, 035–040, 042, 044–047. **Not verified (18):** 004–010, 019–022, 024, 028, 034, 041, 043, 048, **049**. 004–010 are evidence complete and relabelling still owed ([#116](https://github.com/yinggarykairui/factory-hub/issues/116)'s third residual, explicitly *not* a debt candidate under §11); 019–022 and 024 are the paragraph above; 028, 034, 041, 043, 048 and now 049 are `meta` ships that cannot get past #65 · **day 049:** the queue's oldest `queued` issue, [#63](https://github.com/yinggarykairui/factory-hub/issues/63) (2026-08-21), the residuals the day-028 shipper left. Three of its five items were already discharged and were recorded as such rather than re-done; item 3 is won't-fix with reasons; item 1 is the ship. §10's sign-off now carries `sha:` and `base:`, and §9.2 says to write the base down in the §4 spec comment where it is taken. **Three critic cycles, two rejections, and the cap spent on one class of defect** — a confident sentence whose premise is false. The canary caught the draft claiming `base:` *is* the previous ship's sha (day 046 shipped `tool-loop-viz` at `764e949`; its evening left `cea938e`, 22 commits later), and then **both critics caught the correction**: a marker absent at `base:` can still be present in the previous ship's tree, because an evening can delete a literal and the revisit restore it. That draft was a licensed route to exactly the irreversible `verified` §11 spends a paragraph preventing. Two further fixes went in after the cap under directive 4, on hygiene's REJECT, and both were about this row's own subject matter: a misquotation of this file, and a KPI correction written in the present tense before step 8 had made it. Shipped at `a3063dd`, base `40faab4` — the first ship whose sign-off carries both.
